...
export function RevenueChart() {
  ...
  return (
    <div className="bg-white border border-[#1A1A1A]/10 rounded-[24px] shadow-sm p-6 sm:p-8 h-[380px] animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h3 className="text-xl font-bold text-[#1A1A1A] font-sans">누적 계약 및 매출 추이</h3>
          <p className="text-sm text-[#1A1A1A]/60 mt-1 font-sans font-medium">월별 계약 목표 및 실제 매출</p>
        </div>
        <div className="flex items-center gap-4 text-xs font-sans font-bold">
          <div className="flex items-center gap-1.5">
            <div className="w-2.5 h-2.5 rounded-full bg-[#1A1A1A]" />
            <span className="text-[#1A1A1A]/80">실제 매출</span>
          </div>
          <div className="flex items-center gap-1.5">
            <div className="w-2.5 h-2.5 rounded-full bg-[#1A1A1A]/40" />
            <span className="text-[#1A1A1A]/80">목표 매출</span>
          </div>
        </div>
      </div>
        ...
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
            ...
            <XAxis
              ...
              axisLine={false}
              tickLine={false}
              tick={{ fill: "#1A1A1A", fontSize: 12, fontWeight: 600, fontFamily: "sans-serif" }}
              dy={10}
            />
            <YAxis
              axisLine={false}
