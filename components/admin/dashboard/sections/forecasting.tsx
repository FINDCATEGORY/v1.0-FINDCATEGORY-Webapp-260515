  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
        <div>
          <h2 className="text-xl font-semibold text-foreground">매출 예측</h2>
          <p className="text-sm text-muted-foreground mt-1">
            과거 데이터 및 파이프라인 분석 기반 AI 예측
          </p>
        </div>
        <div className="flex items-center gap-3">
          <Select value={timeframe} onValueChange={setTimeframe}>
            ...
            <SelectContent>
              ...
              <SelectItem value="annual">연별</SelectItem>
            </SelectContent>
          </Select>
        ...
        {[
          {
            ...
            subtext: `목표: $${(currentQuarterTarget / 1000000).toFixed(1)}M`,
            icon: Target,
            trend: "+17%",
            trendUp: true,
          },
            ...
            <CardContent className="p-4">
              <div className="flex items-start justify-between">
                ...
                <div className="flex flex-col items-end gap-2">
                  <stat.icon
                    className={`w-5 h-5 ${
                      stat.label === "위험 매출" ? "text-chart-3" : "text-accent"
                    }`}
                  />
                  <Badge
                    variant="outline"
      ...
      <Card className="border-border bg-card">
        <CardHeader className="pb-2">
          <div className="flex items-center justify-between">
            <CardTitle className="text-base font-medium">매출 예측 대 실제</CardTitle>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="border-border bg-card">
          <CardHeader className="pb-2">
            <CardTitle className="text-base font-medium">분기별 예측 분석</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="h-[250px]">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={quarterlyForecast} barGap={4}>
                  ...
                  />
                  <Bar dataKey="committed" name="확정" fill="oklch(0.7 0.18 145)" radius={[4, 4, 0, 0]} />
                  <Bar dataKey="bestCase" name="최상의 경우" fill="oklch(0.7 0.18 220)" radius={[4, 4, 0, 0]} />
                  <Bar dataKey="pipeline" name="파이프라인" fill="oklch(0.22 0.005 260)" radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </CardContent>
        ...
        <Card className="border-border bg-card">
          <CardHeader className="pb-2">
            <CardTitle className="text-base font-medium">시나리오 분석</CardTitle>
          </CardHeader>
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    />
                    <div>
                      <p className="font-medium text-foreground">{scenario.name === "Conservative" ? "보수적" : scenario.name === "Base Case" ? "기본" : "낙관적"}</p>
      ...
      <Card className="border-border bg-card">
        <CardHeader className="pb-2">
          <div className="flex items-center justify-between">
            <CardTitle className="text-base font-medium">위험 요소</CardTitle>
            <Badge variant="outline" className="text-chart-3 border-chart-3/30">
              <AlertTriangle className="w-3 h-3 mr-1" />
              {riskFactors.length}건 발견
            </Badge>
          </div>
        </CardHeader>
        <CardContent>
